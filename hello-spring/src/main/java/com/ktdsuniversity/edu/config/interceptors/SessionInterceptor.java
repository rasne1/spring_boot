 ...
 * 9. 회원 탈퇴 처리 URL
 * 10. 회원 로그아웃 URL
 * ==> 위 URL 10개에 세션 없이 접근할 경우 (컨트롤러가 실행되기 전) 로그인 페이지로 이동시키기.
 * 로그인이 필요없는(세션 데이터가 필요없는) URL
 * 1. 회원가입 페이지
 * 2. 회원가입 처리 URL
 * 3. 로그인 페이지
 * 4. 로그인 처리 URL
 ...
 * 7. 게시글 내용 조회 페이지
 * 8. 이메일 중복검사 URL
 * ==> 위 URL 5개 중 1~4까지는 세션이 있는 상태에서 접근을 못해야 한다.
 * 
 * */
...
public class SessionInterceptor
	 ...
	 * @return 컨트롤러의 실행 여부를 반환.(true: 컨트롤러를 실행한다!, false: 컨트롤러를 실행하지않는다)
	 * */
	@Override
	...
	/*
	 * jsp를 html로 반환시켜 브라우저(클라이언트)로 반환하기 직전에 수행되는 영역.
