...
/*
 * 이클래스의 객체가 Security Filter Chain에 등록되어
 * 인증이 필요한 모든 End Point 가 실행되기 전에 
 * AuthenticationToken(UsernamePasswordAuthenticationToken)을 생성하도록 하는 필터.
 * 
 * HttpServletRequest 의 header로 전달된 Authorization 에 들어 있는 
 * JWT를 가져와 분석(복호화 및 검증) 을 진행하고 분석된 결과를
 * AuthenticationToken 으로 생성 시킨다.
 * */

//OncePerRequestFilter 상속받으면 filter chain에 들어갈수있다
public class JsonWebTokenAuthenticationFilter extends OncePerRequestFilter {
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
		...
		// http://localhost:8080/api/articles?pageNO=1
		// requestUri ==> /api/articles
		
		if(requestURI.startsWith("/api")) {
				// Request에서 header 에 있는 Authorization을 꺼내온다.
		...
		filterChain.doFilter(request, response); // 이 다음 필터가 있다면 그필터를 동작시킨다.
