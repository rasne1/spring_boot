...
public class UsernameAndPasswordAuthenticationProvider implements AuthenticationProvider {
	...
	/*
	 * 사용자가 로그인할 때 전송한 아이돌 회원의 정보를 조회.
	 * */
	private UserDetailsService userDetailsService;
	
	/*
	 * 사용자가 로그인 할때 전송한 비밀번호와 회원의 비밀번호를 비교.
	 * */
	...
	/*
	 * 사용자로부터 Spring Security 로그인 요청이 있을 때마다 실행.
	 * 
	 * 사용자가 보내준 아이디와 비밀번호를 이용해 인증을 수행한다.
	 * UserDetailsService 인터페이스를 이용해 사용자의 정보를 조회하고
	 * PasswordEncoder 인터페이스를 이용해 사용자의 비밀번호를 검증하고
	 * 인증 정보가 일치 할때 만 UsernamepasswordAuthenticationToken을 발급한다.
	 * 
	 * @Param authentication: 사용자가 로그인 요청 한 정보 (아이디, 비밀번호)
